...
<!--    <div v-for="section in sections" :key="section.id">-->
      ...
      <form @submit.prevent="onSubmit" v-if="show.show1 && show.show2 === sectionProps.id" class="mb-3">
        <div class="form-group">
          <label>Título de la sección</label>
          <input type="text" name="name" :defaultValue="sectionProps.name" v-model="sectionDataUpdate.name" class="form-control" placeholder="Introduce el nombre de la sección"/>
          <small class="form-text text-muted">El título no debe de ser demasiado largo</small>
        </div>
        <div class="form-group">
          <label>Orden de la sección</label>
          <input type="number" name="order" :defaultValue="sectionProps.order" v-model="sectionDataUpdate.order" class="form-control" placeholder="Introduce el orden de la sección"/>
...
export default {
  ...
  setup(props) {
    ...
    return {show, changeShow, setHidden, sectionProps, sectionDataUpdate, onSubmit, onFileSelected}
  }
}
</script>
