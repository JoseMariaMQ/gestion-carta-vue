<template>
  <header class="card-header text-center alert alert-danger">
    ...
    <a v-for="contact in contacts" :key="contact.id" :href="contact.url" target="_blank" rel="noreferrer" class="mx-2">
      <FontAwesomeIcon v-if="contact.name.toLowerCase() === 'whatsapp' && contact.url" :icon="['fab', 'whatsapp-square']" class="display-6"/>
      <FontAwesomeIcon v-if="contact.name.toLowerCase() === 'phone' && contact.url" :icon="['fas', 'phone-square']" class="display-6"/>
      <FontAwesomeIcon v-if="contact.name.toLowerCase() === 'facebook' && contact.url" :icon="['fab', 'facebook-square']" class="display-6"/>
